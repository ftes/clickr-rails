...
class BulmaFormBuilder < ActionView::Helpers::FormBuilder
  ...
  def text_field_with_label(method, options = {})
    text_field(method, options) { label(method) }
  end
  ...
  def number_field_with_label(method, options = {})
    number_field(method, options) { label(method) }
  end
  ...
  def email_field_with_label(method, options = {})
    email_field(method, options) { label(method) }
  end
  ...
  def password_field_with_label(method, options = {})
    password_field(method, options) { label(method) }
  end
  ...
  def select(method, choices = nil, options = {}, html_options = {})
    ...
    wrapper.call do
      (block_given? ? yield : nothing) +
        div_control { div_select { super } + errors(method) }
    end
  end

  # Omit &block (custom option rendering)
  def select_with_label(method, choices = nil, options = {}, html_options = {})
    select(method, choices = nil, options, html_options) { label(method) }
  end
  ...
  def collection_select_with_label(
    method,
    collection,
    value_method,
    text_method,
    ...
    wrapper.call do
      div_control do
        check_box(method, options = {}, checked_value, unchecked_value) +
          parent_label(method, nil, { class: 'checkbox m-l-5' })
      end
    end
  end
  ...
  def div_control
    @template.content_tag(:div, class: 'control') { yield }
  end

  def div_field
    @template.content_tag(:div, class: 'field') { yield }
  end

  def div_select
    @template.content_tag(:div, class: 'select') { yield }
  end

  def div_control_for_icons
    @template.content_tag(:div, class: 'control has-icons-left') { yield }
  end
